...
struct FixedPointView: View {
    ...
    static func f4(_ x: Double) -> Double { return -x + 1 }
    ...
    var body: some View {
        ...
        ZStack {
            ...
            // Diagonale y = x
            FunctionDrawing(f: { x in xScreen(x) - graphSize/2 }, integrF: { x in pow(x,2)/2 }, scale: scale)
                .stroke(Color.gray.opacity(1), style: StrokeStyle(lineWidth: 1, dash: [4,3]))
            FunctionDrawing(f: { x in xScreen(functions[selectedFunction](x)) - graphSize/2 }, integrF: { x in pow(x,2)/2 }, scale: scale)
                .stroke(lineWidth: 1)
